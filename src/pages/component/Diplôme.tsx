...
const Req = async () => {
    try {
      const response = await axiosInstance.get<Diplôme>(`https://localhost:7113/api/DiplomeEmploye/GetDiplomesByMatricule/${Matricule}`)
      const tableau = response.data;
      idD =tableau.idDiplome
      idI=tableau.idInstitution
      setDataObject(tableau);

      const resp = await axiosInstance.get<Diplôme>(`https://localhost:7113/api/Diplomes/${idD}`)
      Dip=resp.data.libelleDiplome
      const responseIn= await axiosInstance.get<Diplôme>(`https://localhost:7113/api/Institution/${idI}`)
      Instituion = responseIn.data.libelleInstitution
      const TabDip = await axiosInstance.get<Diplôme[]>(`https://localhost:7113/api/DiplomeEmploye`)
      TabD=TabDip.data
      const TabIns = await axiosInstance.get<Diplôme[]>(`https://localhost:7113/api/DiplomeEmploye`)
      TabI=TabIns.data
    } catch (error) {
  ...
  const Update= async (Data:Diplôme)=>{
    ...
    try{
      const response = await axiosInstance.put(`https://localhost:7113/api/DiplomeEmploye/${Matricule}`,{matricule:Matricule,retraite:true,...Data})
  ...
  const Add =async ()=>{
    try {
      const res = await axiosInstance.post(`https://localhost:7113/api/DiplomeEmploye/add`)
    }catch(error){
      console.error("Erreur:" + error)
    }
  }
...
<>               <List className="timeline"sx={{ width: '100%', bgcolor: 'background.paper',padding:"1rem" , maxHeight:"40dvh",overflowY:"scroll"}}>
    ...
    <ListItem
      ...
      >
        ...
        <ListItemText className={style.SemiBold} primary={`${Dip}`+'. '+`${dataObject?.anneeObtention}`} />
      </ListItem>)
      ...
      case 'Mod':
        ...
        <form  onSubmit={Soumission} style={{overflow:"hidden",padding:"1rem"}} >
          <div style={{display:'flex',flexWrap:"nowrap",marginBottom:"1rem"}}>
            <label  style={{width:"30%"}} htmlFor="Retarite" className={style.SemiBold}>Diplôme & Niveau d'etude</label>
                
        <select name="idDiplome" onChange={handleSelsect} style={{width:"100%",fontSize:"0.9rem"}} >
        {TabD.map((DipObject: Diplôme)=>{
          return <option value={DipObject.idDiplome} selected={DipObject.idDiplome=== idD ? true : false }>{DipObject.libelleDiplome}</option>
        })}
          </select>
          </div>
          <div style={{display:'flex',flexWrap:"nowrap",marginBottom:"1rem"}}>
            <label  style={{width:"30%"}} htmlFor="Retarite" className={style.SemiBold}>Spécialité</label>
            <input  name="specialite" style={{width:"100%"}} type="text" placeholder={dataObject?.specialite} onChange={handleChangeInput} />
          </div>
          <div style={{display:'flex',flexWrap:"nowrap",marginBottom:"1rem"}}>
            <label  style={{width:"30%"}} htmlFor="Retarite" className={style.SemiBold}>Année d'obtention</label>
            <input  style={{width:"100%"}} name='anneeObtention' type="text"  placeholder={dataObject?.anneeObtention} onChange={handleChangeInput} />
          </div>
          <div style={{display:'flex',flexWrap:"nowrap",marginBottom:"1rem"}}>
            <label  style={{width:"30%"}} htmlFor="Retarite" className={style.SemiBold}>Institut</label>
            <select name="idInstitution" onChange={handleSelsect} style={{width:"100%",fontSize:"0.9rem"}} >
        {TabI.map((iObject: Diplôme)=>{
          return <option value={iObject.idInstitution} selected={iObject.idInstitution=== idI ? true : false }>{iObject.libelleInstitution}</option>
        })}
          </select>
          </div>
    ...
    case 'Ajou':
      return (<>
        <form  onSubmit={Ajouter} style={{overflow:"hidden",padding:"1rem"}} >
          <div style={{display:'flex',flexWrap:"nowrap",marginBottom:"1rem"}}>
            <label  style={{width:"30%"}} htmlFor="Retarite"  className={style.SemiBold} >Diplôme & Niveau d'etude</label>
            <select name="idDiplome" onChange={inputSelect} required style={{width:"100%",fontSize:"0.9rem"}} >
            {TabD.map((DipObject: Diplôme)=>{
          return <option value={DipObject.idDiplome}>{DipObject.libelleDiplome}</option>
        })}
        </select>
          </div>
          <div style={{display:'flex',flexWrap:"nowrap",marginBottom:"1rem"}}>
          ...
          <div style={{display:'flex',flexWrap:"nowrap",marginBottom:"1rem"}}>
            <label  style={{width:"30%"}} htmlFor="Retarite"  className={style.SemiBold}>Institut</label>
            <select name="idInstitution" onChange={inputSelect} required style={{width:"100%",fontSize:"0.9rem"}} >
            {TabI.map((instituObject: Diplôme)=>{
          return <option value={instituObject.idInstitution}>{instituObject.libelleInstitution}</option>
        })}
        </select>
          </div>
...
const AfficherCarr = ()=> {
 ...
 <TableContainer component={Paper}>
  <Table aria-label="simple table">
    <TableHead>
      <TableRow>
        <TableCell style={{ border: '1px solid black', width: '10%' }} align='center'  className={style.SemiBold}>Diplôme & Niveau d'etude</TableCell>
        <TableCell style={{ border: '1px solid black', width: '15%' }} align="center"  className={style.SemiBold}>Spécialité</TableCell>
        <TableCell style={{ border: '1px solid black', width: '20%' }} align="center"  className={style.SemiBold}>Année d'obtention</TableCell>
        <TableCell style={{ border: '1px solid black', width: '15%' }} align="center"  className={style.SemiBold}>Institut</TableCell>
    ...
    <TableBody>
        ...
        <TableRow>
          ...
          <TableCell style={{ border: '1px solid black' }} className={style.Regular}align="center">{dataObject?.specialite}</TableCell>
